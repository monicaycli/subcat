...
ge.table %<>% 
  rename(c(Raw.Score = "gm.rcomp.raw", GE = "gm.rcomp.grade")) %>%
...
diag(data_cormtx) <- ''
# write.csv(data_cormtx, "correlation_matrix_r.csv")
...
# write.csv(data_composite.st_cormtx, "composite_correlation_matrix_pearson.csv")
...
rank_cor <- rcorr(as.matrix(data_composite.st), type = "spearman")
rank_cor_mat <- data.frame(rank_cor$r)
rank_cor_mat <- sapply(rank_cor_mat,
                      function(val) { sub("^(-?)0.", "\\1.", sprintf("%.2f", val)) })
rank_cor_mat[upper.tri(rank_cor_mat)] <- ""
diag(rank_cor_mat) <- ""
# write.csv(rank_cor_mat, "composite_correlation_matrix_spearman.csv")
...
## Trial abortion information
  * a trial would be aborted by the experimenter if one of the following steps was not correctly executed:
...
```{r GCA visualization by group, echo=FALSE, fig.width=28, fig.height=10}
# levels(data.trg.allCon$groups.phono.3) <- c("low (0-33.33%), n = 18", "mid (33.33-66.67%), n = 22", "high (66.67-100%), n =21")
p_GCA_groups <-
ggplot(data.trg.allCon, aes(Time, meanFix, shape = COND)) +
...
```{r fix prop by item and by POA_PAIR, include=FALSE, eval=FALSE}
...
gazeItem.s$POA_PAIR <- NA
gazeItem.s[gazeItem.s$TARGET %in% c("tap"),]$POA_PAIR <- "LA"
gazeItem.s[gazeItem.s$TARGET %in% c("net","bud","butt","fort","bat","pit","hood","rod"),]$POA_PAIR <- "AL"
gazeItem.s[gazeItem.s$TARGET %in% c("carp","harp"),]$POA_PAIR <- "LV"
gazeItem.s[gazeItem.s$TARGET %in% c("beak"),]$POA_PAIR <- "VL"
gazeItem.s[gazeItem.s$TARGET %in% c("cat","road","knot"),]$POA_PAIR <- "AV"
gazeItem.s$POA_PAIR <- factor(gazeItem.s$POA_PAIR, levels=c("LA","AL","LV","VL","AV"))
...
# plot by POA_PAIR
# ggplot(subset(gazeItem.s, FIX_OBJECT=="target" & Time >= 600 & Time <= 1200), 
#        aes(Time, meanFix, shape=COND)) + 
#   facet_grid(POA_PAIR+TARGET~.) +
#   stat_summary(fun.y=mean, geom="line", aes(color=COND)) +
#   stat_summary(fun.y=mean, geom="point", aes(color=COND)) +
#   stat_summary(fun.data=mean_se, geom="ribbon", alpha=.2, aes(fill=COND)) +
...
```{r save image}
save.image("Subcat_Results_Manuscript.RData")
```